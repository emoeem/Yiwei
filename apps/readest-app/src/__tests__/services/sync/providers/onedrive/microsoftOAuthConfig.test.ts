...
describe('microsoftOAuthConfig', () => {
  test('builds the Microsoft OAuth config with the custom-scheme redirect', () => {
    ...
    expect(cfg.redirectUri).toBe('yiwei-onedrive://auth');
    expect(cfg.redirectScheme).toBe('yiwei-onedrive');
    expect(cfg.authParams).toEqual({ prompt: 'select_account' });
  });

  test('isOneDriveOAuthRedirectUrl flags the OneDrive redirect scheme', () => {
    expect(isOneDriveOAuthRedirectUrl('yiwei-onedrive://auth?code=x&state=y')).toBe(true);
    expect(isOneDriveOAuthRedirectUrl('yiwei://auth-callback')).toBe(false);
