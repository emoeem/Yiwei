 ...
 * `yiwei://` app deeplink or the universal `https://` web link.
 */
...
export const buildAnnotationAppUrl = ({ bookHash, noteId, cfi }: AnnotationDeepLink): string => {
  const base = `yiwei://book/${bookHash}/annotation/${noteId}`;
  return cfi ? `${base}?cfi=${encodeURIComponent(cfi)}` : base;
};
...
/**
 * Parse an incoming yiwei:// or https://web.readest.com annotation URL.
...
export const parseAnnotationDeepLink = (url: string): AnnotationDeepLink | null => {
  ...
  const isCustomScheme = parsed.protocol === 'yiwei:';
  const isWebHost =
    (parsed.protocol === 'https:' || parsed.protocol === 'http:') &&
    parsed.host === 'web.readest.com';
  if (!isCustomScheme && !isWebHost) return null;

  // For yiwei:// URLs the URL parser stores the first path segment in the
...
/**
 * Parse an incoming yiwei:// or https://web.readest.com book-open URL.
