...
final class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
  ...
  private func makeRootTemplate() -> CPListTemplate {
    ...
    let active = defaults.bool(forKey: "yiwei.carplay.active")

    let item: CPListItem
    if active {
      let title = defaults.string(forKey: "yiwei.carplay.title") ?? "Now Reading"
      let author = defaults.string(forKey: "yiwei.carplay.author") ?? ""
    ...
    return CPListTemplate(title: "Yiwei", sections: [section])
  }
}
